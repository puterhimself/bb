...
Source: https://linuxcontainers.org/incus/docs/main/howto/images_create/
Fetched: 2026-08-07
...
```
In both cases, you can specify an alias for the new image with the
--alias
flag, set an expiration date with
--expire
and make the image publicly available with
--public
.
If an image with the same name already exists, add the
--reuse
...
template
to edit)
Instance-specific data inside the instance itself (for example, host SSH keys and
dbus/systemd
machine-id
)
Build an image
¶
For building your own images, you can use
distrobuilder
.
See the
distrobuilder
documentation
for instructions for installing and using the tool.
