...
Source: https://linuxcontainers.org/incus/docs/main/instance-exec/
Fetched: 2026-08-07
...
incus-agent
process must be running inside of the virtual machine for this to work.
...
/bin/bash
), you can get shell access to your instance.
...
root
(UID 0) with the default group (GID 0) and the working directory set to
/root
.
You can override the user, group and working directory by specifying absolute values through the following flags:
--user
- the user ID for running the command
--group
- the group ID for running the command
--cwd
...
/bin/bash
command exists in your instance):
```
