...
Source: https://linuxcontainers.org/incus/docs/main/howto/instances_routed_nic_vm/
Fetched: 2026-08-07
...
When adding a
routed NIC device
to an instance, you must configure the instance to use the link-local gateway IPs as default routes.
For containers, this is configured for you automatically.
For virtual machines, the gateways must be configured manually or via a mechanism like
cloud-init
.
To configure the gateways with
cloud-init
...
netplan
configuration to the instance using the
...
static link-local next-hop addresses
(
169.254.0.1
and
fe80::1
) that are required.
For each of these routes we set
on-link
to
true
...
```
Note
Before you start your instance, make sure that you have
configured the parent network
to enable proxy ARP/NDP.
