...
Source: https://linuxcontainers.org/incus/docs/main/profiles/
Fetched: 2026-08-07
...
If you don’t specify any profiles when launching a new instance, the
default
...
¶
To set an instance option for a profile, use the
incus
profile
set
command.
...
```
To add and configure an instance device for your profile, use the
incus
profile
device
add
command.
Specify the profile name, a device name, the device type and maybe device options (depending on the
device type
):
```
...
```
To configure instance device options for a device that you have added to the profile earlier, use the
incus
profile
device
set
...
```
Instance options are provided as an array under
config
.
Instance devices and instance device options are provided under
devices
.
...
```
Alternatively, you can create a YAML file (for example,
profile.yaml
...
```
Tip
Check the configuration after adding the profile:
incus
config
show
<instance_name>
You will see that your profile is now listed under
profiles
.
However, the configuration options from the profile are not shown under
config
(unless you add the
--expanded
flag).
...
You can also specify profiles when launching an instance by adding the
--profile
flag:
```
