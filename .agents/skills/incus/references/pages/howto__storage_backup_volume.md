...
Source: https://linuxcontainers.org/incus/docs/main/howto/storage_backup_volume/
Fetched: 2026-08-07
...
If you have a separate, network-connected Incus server available, regularly copying volumes to this other server gives high reliability as well, and this method can also be used to back up snapshots of the volume.
Note
Custom storage volumes might be attached to an instance, but they are not part of the instance.
Therefore, the content of a custom storage volume is not stored when you
...
To do so, set the
snapshots.schedule
configuration option for the storage volume (see
...
See the
Storage drivers
documentation for more information about those configuration options.
...
¶
Use the following command to export a custom storage volume to a compressed file (for example,
...
/path/to/my-backup.tgz
) as a new custom storage volume.
To do so, use the following command:
```
