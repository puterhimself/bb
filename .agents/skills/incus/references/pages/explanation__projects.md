...
Source: https://linuxcontainers.org/incus/docs/main/explanation/projects/
Fetched: 2026-08-07
...
for instructions on how to add projects.
...
default
project.
To configure which entities are isolated, you enable or disable the respective
feature
in the project.
If a feature is enabled, the corresponding entity is isolated in the project; if the feature is disabled, it is inherited from the
default
project.
For example, if you enable
features.networks
for a project, the project uses a separate set of networks and not the networks defined in the
default
project. If you disable
features.images
, the project has access to the images defined in the
default
project, and any images you add while you’re using the project are also added to the
default
project.
See the list of available
Project features
for information about which features are enabled or disabled when you create a project.
Note
...
This method isolates the instances and other entities (depending on the feature configuration), as described in
Isolation of projects
.
...
In addition, this method allows users to work with Incus without being a member of the
incus-admin
group (see
...
Members of the
incus-admin
group have full access to Incus, including permission to attach file system paths and tweak the security features of an instance, which makes it possible to gain root access to the host system.
...
to allow access to specific projects only.
...
This is usually used for users that are a member of the
incus
group but aren’t in the more privileged
incus-admin
group.
...
for instructions on how to enable and configure the different authentication methods.
