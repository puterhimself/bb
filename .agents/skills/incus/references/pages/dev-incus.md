...
Source: https://linuxcontainers.org/incus/docs/main/dev-incus/
Fetched: 2026-08-07
...
In Incus, this feature is implemented through a
/dev/incus/sock
node which is
...
must be set to
true
(which is the default) for an instance to allow access to the socket.
Implementation details
¶
Incus on the host binds
/var/lib/incus/guestapi/sock
and starts listening for new
connections on it.
...
Queries on
/dev/incus/sock
will only return information related to the
...
Return: list of supported API endpoint URLs (by default
['/1.0']
)
Return value:
```
...
PATCH
¶
Description: Update instance state (valid states are
Ready
and
Started
)
...
and
user.*
keys are accessible to the instance.
...
configuration keys)
device
(any device addition, change or removal)
