...
Source: https://linuxcontainers.org/incus/docs/main/howto/images_manage/
Fetched: 2026-08-07
...
You can also export an image to a file, which can be useful to
copy or import it
on another machine.
...
```
If you do not specify a remote, the
default remote
is used.
...
```
To filter for properties other than alias or fingerprint, specify the filter in
<key>=<value>
format.
For example:
```
...
For a remote image, remember to include the remote server (for example,
images:debian/12
).
...
```
You can also display a specific image property (located under the
properties
...
¶
To set a specific image property that is located under the
properties
...
```
You can directly assign an alias to an image when you
copy or import
or
publish
it.
...
```
To export a virtual machine image to a file, add the
--vm
flag:
```
...
See
Image format
for a description of the file structure used for the image.
