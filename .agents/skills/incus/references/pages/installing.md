...
Source: https://linuxcontainers.org/incus/docs/main/installing/
Fetched: 2026-08-07

How to install Incus
¶
The easiest way to install Incus is to
install one of the available packages
, but you can also
install Incus from the sources
.
After installing Incus, make sure you have an
incus-admin
...
Ubuntu
Void Linux
Incus and all of its dependencies are available in Alpine Linux’s edge main and community repository as
incus
.
Uncomment the edge main and community repositories in
...
```
Please report packaging issues
here
.
Incus and all of its dependencies are available in Arch Linux’s main repository as
incus
.
Install Incus with:
```
...
```
See also
the Incus documentation page at Arch Linux
for more details about the installation, configuration, use and troubleshooting.
Please report packaging issues
here
.
Incus and its dependencies are available in Chimera Linux’s
user
repository as
incus
...
Then add the
incus
package; this will install other dependencies including
incus-client
. Enable the service.
```
apk add incus
...
and
incus-base
packages are available beginning with the Debian 13 (
trixie
) release.
...
On Debian systems, running
apt
install
incus
will get Incus installed with all dependencies required for running containers and virtual machines.
If you only wish to run containers in Incus, you can run just
apt
install
incus-base
.
...
Zabbly
provides up to date and supported Incus packages for Debian 13 (
trixie
), 12 (
bookworm
) and 11 (
...
Docker/Podman images of Incus, based on the Zabbly package repository, are available with instructions here:
...
here
.
Incus and all of its dependencies are available in Gentoo’s main repository as
...
incus
to allow basic user access (launch containers), and
incus-admin
for
incus
admin
controls. Add your regular users to either, or both, depending on your setup and use cases.
...
Gentoo Wiki
.
Incus and its dependencies are packaged in NixOS and are configurable through NixOS options. See
...
```
Incus initialization can be done manually using
incus
admin
init
, or through the preseed option in your NixOS configuration. See the NixOS documentation for an example preseed.
```
virtualisation.incus.preseed = {};

```
Finally, you can add users to the
incus-admin
group to provide non-root access to the Incus socket. In your NixOS configuration:
```
users.users.YOUR_USERNAME.extraGroups = ["incus-admin"];

```
Instead of giving the users a full Incus daemon access, you can add users to the
incus
group, which will only grant access to the Incus user socket. In your NixOS configuration:
```
users.users.YOUR_USERNAME.extraGroups = ["incus"];

```
For any NixOS specific issues, please
file an issue
...
```
The default setup should work fine for most users, but if you intend to run many containers on your system you may wish to apply some custom
sysctl
settings
as suggested in the production deployments guide
.
Please report packaging issues
here
.
Make sure to mark the bug as being in the “Containers” component, to make sure the right package maintainers see the bug.
RPM packages and their dependencies are not yet available from the Extra Packages for Enterprise Linux (EPEL) repository, but via the
neelc/incus
Community Project (COPR) repository for Rocky Linux 9 and 10.
...
(
CRB
) is available for other package dependencies:
```
...
```
CRB
is not required on Rocky Linux 10.
...
A native
incus
package is currently available in Ubuntu 24.04 LTS and later.
On such systems, just running
apt
install
incus
will get Incus installed.
To run virtual machines, also run
apt
install
qemu-system
...
Zabbly package repository
Zabbly
provides up to date and supported Incus packages for Ubuntu LTS releases (22.04, 24.04, and 26.04).
Those packages contain everything needed to use all Incus features.
Up to date installation instructions may be found here:
https://github.com/zabbly/incus
Incus and all of its dependencies are available in Void Linux’s repository as
incus
.
Install Incus with:
```
...
Windows
Homebrew
Incus publishes builds of the Incus client for macOS through
Homebrew
.
...
```
Colima
Incus is supported as a runtime on
Colima
.
Install Colima with:
```
...
```
For any Colima related issues, please
file an issue
in the project repository.
The Incus client on Windows is provided as a
Chocolatey
and
Winget
package.
...
Chocolatey
Install Chocolatey by following the
...
```
Winget
Install Winget by following the
installation instructions
Install the Incus client:
```
...
```
You can also find native builds of the Incus client on
GitHub
:
Incus client for Linux:
bin.linux.incus.aarch64
,
bin.linux.incus.x86_64
Incus client for Windows:
bin.windows.incus.aarch64.exe
,
bin.windows.incus.x86_64.exe
Incus client for macOS:
...
We recommend having the latest versions of
liblxc
(>= 5.0.0 required)
available for Incus development. Additionally, Incus requires a modern Golang (see
Go
)
version to work.
Alpine Linux
...
You can reference the list of packages you need to use specific functions from
LXD package definition in Alpine Linux repository
.
Also you can find the package you need with the binary name from
Alpine Linux packages contents filter
.
...
NOTE:
Some build errors may occur if
/usr/local/include
doesn’t exist on the system.
Also, due to a
gettext
issue
, you may need to set those additional environment variables:
```
...
The version of
golang-go
in your version of Debian or Ubuntu may not be sufficient to build Incus (see
Go
).
In such cases, you may need to install a newer Go version
from upstream
.
There are a few storage drivers for Incus besides the default
dir
driver.
...
If you use the
liblxc-dev
package and get compile time errors when building the
go-lxc
module,
...
, replace it with
0
to work around the problem. It’s a packaging bug, and
...
by Incus – and
make
, which builds Incus itself. At the end of
make
deps
, a message will be displayed which will specify environment variables that should be set prior to invoking
make
. As new versions of Incus are released, these environment
variable settings may change, so be sure to use the ones displayed at the end of the
make
deps
process, as the ones
...
LD_LIBRARY_PATH="$(go
env
GOPATH)/deps/cowsql/.libs/:$(go
env
GOPATH)/deps/raft/.libs/:${LD_LIBRARY_PATH}"
#
export
...
From source: Install
¶
Once the build completes, you simply keep the source tree, add the directory referenced by
$(go
env
GOPATH)/bin
to
your shell path, and set the
LD_LIBRARY_PATH
variable printed by
make
deps
...
and
incus
binaries will be available to you and can be used to set up Incus. The binaries will automatically find and use the dependencies built in
$(go
env
GOPATH)/deps
thanks to the
...
```
Now you can run the daemon (the
--group
sudo
bit allows everyone in the
sudo
...
If
newuidmap/newgidmap
tools are present on your system and
/etc/subuid
,
etc/subgid
...
The root user and all members of the
incus-admin
...
If the
incus-admin
group is missing on your system, create it and restart the Incus daemon.
...
A backup of the database before the update is stored in the same location as the active database (at
