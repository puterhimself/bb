...
Source: https://linuxcontainers.org/incus/docs/main/container-environment/
Fetched: 2026-08-07
...
Devices
¶
Incus containers have a minimal and ephemeral
/dev
based on a
tmpfs
file system.
...
and not a
devtmpfs
file system, device nodes appear only if manually created.
...
¶
Incus sets up a socket at
/dev/incus/sock
that the root user in the container can use to communicate with Incus on the host.
See
...
(
cgroupfs
) (only on kernels that lack cgroup namespace support)
...
/proc
files being overridden through bind-mounts.
PID1
¶
Incus spawns whatever is located at
/sbin/init
as the initial process of the container (PID 1).
...
SIGINT
to trigger a reboot of the container
SIGPWR
(or alternatively
SIGRTMIN
+3) to trigger a clean shutdown of the container
