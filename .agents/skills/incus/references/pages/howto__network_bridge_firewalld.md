...
Source: https://linuxcontainers.org/incus/docs/main/howto/network_bridge_firewalld/
Fetched: 2026-08-07

How to configure your firewall
¶
Linux firewalls are based on
netfilter
.
...
to manage its
netfilter
rules. It places its rules in their own
nftables
namespace to keep them separated from rules added by other applications.
...
To enable or disable this behavior, use the
ipv4.firewall
or
ipv6.firewall
...
See the following sections for instructions on how to disable Incus’ firewall rules and how to properly configure
firewalld
and UFW, respectively.
Disable Incus’ firewall rules
¶
Run the following commands to prevent Incus from setting firewall rules for a specific network bridge (for example,
incusbr0
):
```
...
```
firewalld
: Add the bridge to the trusted zone
¶
To allow traffic to and from the Incus bridge in
firewalld
...
A common reason for these issues is that Docker sets the global FORWARD policy to
drop
, which prevents Incus from forwarding traffic and thus causes the instances to lose network connectivity.
See
Docker on a router
...
This behavior can be changed through the
ip-forward-no-drop
Docker configuration option in
daemon.json
.
Setting the option to
true
may be sufficient to allow both Incus and Docker to operate on the same system.
...
To enable IPv4 forwarding before Docker starts, ensure that the following
sysctl
setting is enabled:
```
...
One way of doing this is to add a file to the
/etc/sysctl.d/
...
incusbr0
, you can allow egress traffic to flow using the following commands:
```
