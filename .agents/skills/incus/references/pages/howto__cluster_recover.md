...
Source: https://linuxcontainers.org/incus/docs/main/howto/cluster_recover/
Fetched: 2026-08-07
...
¶
Every Incus cluster has a specific number of members (configured through
cluster.max_voters
) that serve as voting members of the distributed database.
...
Pick one of the listed database members that is still online as the new leader.
Log on to the machine (if it differs from the one you are already logged on to).
Make sure that the Incus daemon is not running on the machine.
```
sudo systemctl stop incus.service incus.socket

```
Log on to all other cluster members that are still online and stop the Incus daemon.
...
```
Start the Incus daemon again on all machines, starting with the new leader.
```
...
You can edit the
Member roles
of the different members, but with the following limitations:
A cluster member that does not have a
database*
role cannot become a voter, because it might lack a global database.
...
```
Edit the YAML representation of the information that this cluster member has about the rest of the cluster:
```
...
For example, if you have a cluster member that was removed uncleanly, it might not show up in
incus
cluster
list
but still be part of the Raft configuration.
