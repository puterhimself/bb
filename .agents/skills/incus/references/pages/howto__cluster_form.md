...
Source: https://linuxcontainers.org/incus/docs/main/howto/cluster_form/
Fetched: 2026-08-07
...
¶
To form your cluster, you must first run
incus
admin
init
on the bootstrap server. After that, run it on the other servers that you want to join to the cluster.
When forming a cluster interactively, you answer the questions that
incus
admin
init
prompts you with to configure the cluster.
Initialize the bootstrap server
¶
To initialize the bootstrap server, run
incus
admin
init
and answer the questions according to your desired configuration.
...
no
.
Expand to see a full example for
incus admin init
...
) is configured to a specific IP or DNS address using
incus
config
set
core.https_address=[IP_OR_DNS]:8443
. The default wildcard value cannot be used in clustered mode.
Then you can run
incus
cluster
enable
memberName
and continue with the steps below to join in additional cluster members.
...
To join a server to the cluster, run
incus
admin
init
on the cluster.
Joining an existing cluster requires root privileges, so make sure to run the command as root or with
sudo
.
Basically, the initialization process consists of the following steps:
Request to join an existing cluster.
Answer the first questions that
incus
admin
init
asks accordingly:
...
If you configured your cluster to use
authentication tokens
, you must generate a join token for each new member.
...
```
This command returns a single-use join token that is valid for a configurable time (see
...
admin
init
prompts you for the join token.
...
This reduces the amount of questions that you must answer during
incus
admin
init
, because the join token can be used to answer these questions automatically.
Confirm that all local data for the server is lost when joining a cluster.
Configure server-specific settings (see
Member configuration
for more information).
You can accept the default values or specify custom values for each server.
Expand to see full examples for
incus admin init
...
join
token:

All
existing
data
is
...
admin
init
questions interactively, you can provide the required information through preseed files.
You can feed a file to
incus
admin
init
...
Join additional servers
¶
The preseed files for new cluster members require only a
cluster
section with data and configuration values that are specific to the joining server.
Authentication tokens
