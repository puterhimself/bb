...
Source: https://linuxcontainers.org/incus/docs/main/rest-api/
Fetched: 2026-08-07
...
for information about how to access the API remotely.
Tip
For examples on how the API is used, run any command of the Incus client (
incus
) with the
--debug
flag.
The debug information displays the API calls and the return values.
For quickly querying the API, the Incus client provides a
incus
query
command.
API versioning
...
happening.
In most cases, those will be called status and
status_code
, the former
...
logic. Logical operators are also supported for filtering: not (
not
), equals (
eq
),
...
or
). Filters are evaluated with left associativity.
...
API structure
¶
Incus has an auto-generated
Swagger
specification describing its API endpoints.
The YAML version of this API specification can be found in
rest-api.yaml
.
See
REST API
