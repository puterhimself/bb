...
Source: https://linuxcontainers.org/incus/docs/main/howto/network_increase_bandwidth/
Fetched: 2026-08-07
...
txqueuelen
value of 10000, which is commonly used with 10GbE NICs, and a
net.core.netdev_max_backlog
value of 182757.
Depending on your network, you might need to use different values.
In general, you should use small
txqueuelen
values with slow devices with a high latency, and high
txqueuelen
values with devices with a low latency.
For the
net.core.netdev_max_backlog
value, a good guideline is to use the minimum value of the
net.ipv4.tcp_mem
configuration.
...
txqueuelen
) of both the real NIC and the Incus NIC (for example,
incusbr0
).
...
txqueuelen
value for all Ethernet interfaces in your instances.
To do this, use one of the following methods:
Apply the same changes as described above for the Incus host.
Set the
queue.tx.length
device option on the instance profile or configuration.
