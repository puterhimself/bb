...
Source: https://linuxcontainers.org/incus/docs/main/howto/storage_move_volume/
Fetched: 2026-08-07
...
or
move
custom storage volumes from one storage pool to another, or copy or rename them within the same storage pool.
To move instance storage volumes from one storage pool to another,
move the corresponding instance
to another pool.
...
If the volume already exists in the target location, use the
--refresh
flag to update the copy.
...
¶
Before you can move or rename a custom storage volume, all instances that use it must be
stopped
.
...
¶
For most storage drivers (except for
ceph
and
ceph-fs
), storage volumes exist only on the cluster member for which they were created.
To copy or move a custom storage volume from one cluster member to another, add the
--target
and
--destination-target
