...
Source: https://linuxcontainers.org/incus/docs/main/howto/network_ovn_setup/
Fetched: 2026-08-07
...
¶
Complete the following steps to create a standalone OVN network that is connected to a managed Incus parent bridge network (for example,
incusbr0
...
```
Create an instance that uses the
ovntest
network:
```
...
incus
list
to show the instance information:
~$
incus
list
+------+---------+---------------------+-----------------------------------------------+-----------+-----------+
...
(eth0)
|
2001:db8:cff3:5089:1266:6aff:fef0:549f
(eth0)
|
CONTAINER
|
...
for editing.
Paste in one of the following configurations (replace
<server_1>
,
<server_2>
and
<server_3>
with the IP addresses of the respective machines, and
<local>
with the IP address of the machine that you are on).
...
```
On the remaining machines, install only
ovn-host
...
```
Create an Incus cluster by running
incus
admin
init
on all machines.
On the first machine, create the cluster.
Then join the other machines with tokens by running
incus
cluster
add
<machine_name>
on the first machine and specifying the token when initializing Incus on the other machine.
...
for instructions on how to set up this bridge.
Gateway
Run
ip
-4
...
Still on the first machine, configure Incus to be able to communicate with the OVN DB cluster.
To do so, find the value for
ovn-northd-nb-db
in
/etc/default/ovn-central
and provide it to Incus with the following command:
```
...
incus
monitor
to see logged network ACL traffic from the OVN controller:
```
