...
Source: https://linuxcontainers.org/incus/docs/main/howto/images_copy/
Fetched: 2026-08-07
...
local:
as the target remote.
See
incus
image
copy
--help
for a list of all available flags.
...
¶
If you have image files that use the required
Image format
, you can import them into your image store.
There are several ways of obtaining such image files:
Exporting an existing image (see
...
Downloading image files from a
remote image server
(note that it is usually easier to
use the remote image
directly instead of downloading it to a file and importing it)
Import from the local file system
¶
To import an image from the local file system, use the
incus
image
import
command.
...
unified images
(compressed file or directory) and
split images
(two files).
...
It only requires a basic web server with support for custom headers (see
Custom HTTP headers
).
The image files must be provided as unified images (see
Unified tarball
).
