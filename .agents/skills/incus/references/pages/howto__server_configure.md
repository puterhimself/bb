...
Source: https://linuxcontainers.org/incus/docs/main/howto/server_configure/
Fetched: 2026-08-07
...
for all configuration options that are available for the Incus server.
...
In the
Server configuration
option tables, options that apply to the cluster are marked with a
global
scope, while options that apply to the local server are marked with a
local
scope.
...
```
In a cluster setup, to configure a server option for a cluster member only, add the
--target
flag.
...
```
In a cluster setup, to show the local configuration for a specific cluster member, add the
--target
flag.
...
```
In a cluster setup, to edit the local configuration for a specific cluster member, add the
--target
flag.
