...
Source: https://linuxcontainers.org/incus/docs/main/debugging/
Fetched: 2026-08-07

How to debug Incus
¶
For information on debugging instance issues, see
...
Adding
--debug
flag to any client command will give extra information
...
¶
HTTPS connection to Incus
requires valid
client certificate that is generated on first
incus
remote
add
. This
...
If desired,
openssl
can be used to examine the certificate (
...
incus
client certificate into importable format and import it into
browser.
For example this produces
client.pfx
in Windows-compatible format:
```
...
¶
The files of the global
database
are stored under the
./database/global
sub-directory of your Incus data directory (
...
Please consult the Incus team first (creating a
GitHub
issue
or
forum
...
creating
.sql
files containing queries that repair the broken update.
To perform repairs against the local database, write a
./database/patch.local.sql
file containing the relevant queries, and
similarly a
./database/patch.global.sql
for global database repairs.
...
global
.sync
command, that will write a plain SQLite database file into
./database/global/db.bin
, which you can then inspect with the
sqlite3
command line tool.
